...
// Cross-module integration: the user use-cases and the post repo working
// together against a real database, with no GraphQL/HTTP layer (that path is
// covered in e2e/). The integration point is the User←Post relation: a post
// must connect to a user created by the user module.
...
describe('user → post integration', () => {
  ...
  it('rejects a post for a non-existent author (FK enforced)', async () => {
    await expect(postRepo.createPost(prisma, { title: 'orphan', authorId: 999 })).rejects.toThrow();
  });

  it('a published post by the user shows up under onlyPublished', async () => {
    const author = await users.create({ email: 'bob@example.com' });
    const draft = await postRepo.createPost(prisma, { title: 'Draft', authorId: author.id });

    expect(await postRepo.findMany(prisma, {}, { onlyPublished: true })).toHaveLength(0);

    await postRepo.publishPost(prisma, draft.id);
