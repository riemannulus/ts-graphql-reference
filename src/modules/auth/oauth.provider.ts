...
/**
 * Port for talking to a specific OAuth provider (Google here) — a record of
 * functions, not a class. The OAuth service depends on this shape, never on a
 * concrete HTTP client, so the provider integration can be left unimplemented
 * in production and swapped for a fake in tests (an object literal is enough)
 * without touching the routing, parsing, or user-provisioning logic around it.
 */
 ...
 */
export const stubGoogleOAuthClient: GoogleOAuthClient = {
  buildAuthUrl(_state) {
    throw new Error('GoogleOAuthClient.buildAuthUrl not implemented: configure Google OAuth');
  },
  exchangeCode(_code) {
    return Promise.reject(
      new Error('GoogleOAuthClient.exchangeCode not implemented: configure Google OAuth'),
    );
  },
  fetchProfile(_tokens) {
    return Promise.reject(
      new Error('GoogleOAuthClient.fetchProfile not implemented: configure Google OAuth'),
    );
  },
};
