 ...
 * selection client (`ctx.db` — the replica for query operations). No
...
export function registerPointQueries(): void {
  builder.queryField('pointBalance', (t) =>
    t.prismaField({
      ...
      resolve: (query, _root, args, ctx) => pointRepo.findBalance(ctx.db, args.userId, query),
    }),
  );
  ...
  builder.queryField('pointCharges', (t) =>
    t.prismaField({
      ...
      args: { userId: t.arg.int({ required: true }) },
      resolve: (query, _root, args, ctx) => pointRepo.findCharges(ctx.db, args.userId, query),
    }),
  );
  ...
  builder.queryField('pointSpends', (t) =>
    t.prismaField({
      ...
      args: { userId: t.arg.int({ required: true }) },
      resolve: (query, _root, args, ctx) => pointRepo.findSpends(ctx.db, args.userId, query),
    }),
  );
}
