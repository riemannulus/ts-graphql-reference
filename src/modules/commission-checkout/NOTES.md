...
commissionCheckout.complete({ orderPaymentId, actorId, commandKey })
  -> { orderId, orderPaymentId, contractId, reservationId,
       referenceId, commandId, operationId, replayed }
```
...
the product-owned financial link, and persists the replayable
Flow → Command → Operation → Action identity spine.
...
narrowed by dependency-cruiser to the reviewed owner repos plus
`financial-ledger.core` planning and the generic `transaction-flow` core/repo. `financial-ledger` has no product imports and
...
- The financial model is the minimum needed for a POINT hold. The reservation
  ...
  invalidate or rewrite a committed transfer. The implemented PAY operation
  contains a typed TRANSFER action and exposes `COMMISSION-<uuid>`,
  `CMD-PAY-<uuid>`, and `OP-PAY-<uuid>` identifiers. It does not prove
  settlement, refund, withdrawal, accounting, or outbox designs.
...
database, and narrows commission-checkout's direct imports to owner plan/repo APIs. The
latest revision adds typed public identity and DB-enforced command/operation
kind equality. Final
