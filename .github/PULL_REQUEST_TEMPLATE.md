...
- [ ] **Q3 new composite module** — a capability above the owners (a cross-owner transaction, or an external port/protocol over an owner's rows); the new module takes the edges
...
- [ ] `.dependency-cruiser.mjs`: module in the default rule's `pathNot` exemption + its `X-reaches-Y-only` rule added or widened (+ `X-to-Y-is-type-only` for a rung-2 edge)
...
- [ ] Invariant-carrying writes reached across the edge take core-minted types — branded values or plans (§4); no service-guarded write is reached
